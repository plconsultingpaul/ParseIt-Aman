...
export function evaluateDateTimeMergeFunction(logic: DateTimeMergeFunctionLogic, data: Record<string, any>): string {
  ...
  console.log('[DateTimeMerge] dateField=%s dateVal=%s timeField=%s timeVal=%s inputDateFormat=%s outputFormat=%s',
    logic.dateFieldName, JSON.stringify(dateVal), logic.timeFieldName, JSON.stringify(timeVal), logic.inputDateFormat, logic.outputFormat);
  if (!dateParts) {
    console.log('[DateTimeMerge] dateParts=null, returning empty string');
    return '';
  }
  const parsedTime = parseTimePart(timeVal);
  const defaultTime = parseTimePart(logic.emptyTimeDefault);
  const timeParts = parsedTime || defaultTime || { hours: 0, minutes: 0, seconds: 0 };
  if (!parsedTime) {
    console.log('[DateTimeMerge] time value blank/unparseable; emptyTimeDefault=%s -> using %s',
      JSON.stringify(logic.emptyTimeDefault), JSON.stringify(timeParts));
  }
  const result = formatDateTimeMerge(dateParts, timeParts, logic.outputFormat);
  console.log('[DateTimeMerge] dateParts=%s timeParts=%s result=%s', JSON.stringify(dateParts), JSON.stringify(timeParts), result);
  return result;
}
...
export async function evaluateAddressLookupAsync(
  ...
  console.log(`[AddressLookup] Starting async lookup: type=${lookupType}, inputFields=${JSON.stringify(inputFields)}, defaultValue="${defaultValue || ''}"`);

  const addressParts: string[] = [];
  for (const field of inputFields) {
    const value = getFieldValue(field, data);
    console.log(`[AddressLookup] Field "${field}" resolved to: "${value}"`);
...
IMPORTANT: You MUST provide a value. Never respond with "Cannot determine", "Unknown", or any refusal. A city and province/state is sufficient to look up a ${lookupLabel}. Provide your best answer.`;
  ...
  console.log(`[AddressLookup] Prompt: "${prompt}"`);
  ...
  const callGemini = async (promptText: string): Promise<string> => {
    ...
    if (!response.ok) {
      ...
      console.error(`[AddressLookup] Gemini API HTTP error: ${response.status}, body: ${errorText}`);
      throw new Error(`Gemini API error: ${response.status}`);
    }

    const result = await response.json();
    console.log(`[AddressLookup] Gemini raw response: ${JSON.stringify(result.candidates?.[0]?.content?.parts?.[0])}`);
  ...
  try {
    console.log(`[AddressLookup] Calling Gemini API (gemini-2.0-flash)...`);
    let text = await callGemini(prompt);
    console.log(`[AddressLookup] Extracted text: "${text}"`);
    ...
    if (isRefusal(text)) {
      ...
      console.log(`[AddressLookup] Retry extracted text: "${text}"`);
    }

    if (isRefusal(text)) {
      console.log(`[AddressLookup] AI could not resolve after retry (text="${text}"), returning default: "${defaultValue || ''}"`);
      return defaultValue || '';
    }
    console.log(`[AddressLookup] Returning resolved value: "${text}"`);
    return text;
  } catch (error) {
