...
export async function executeAiDecision(
  ...
  if (resultArrayPath && resultArrayPath.trim() !== '') {
    ...
    if (arrayKey && responseData[arrayKey].length > 0) {
      candidates = responseData[arrayKey];
    } else {
      candidates = [responseData];
    }
  }
  ...
  if (candidates.length === 0) {
    if (failOnNoMatch) {
      throw new Error('AI Decision: API returned zero candidates. Cannot match.');
    }
    const stepOutput = {
      url: fullUrl,
      method: httpMethod,
...
Return ONLY the JSON object, no markdown formatting, no code blocks, no extra text.`;
  ...
  if (executionLogId && workflowId) {
    await createV2StepLog(
      ...
      'completed', sub3Start, sub3End, sub3Duration, null,
      { candidateCount: candidates.length, resolvedSourceFields, aiInstruction, model: modelName },
      { selectedIndex: aiResult.matchIndex, confidence: aiResult.confidence, selectionReason: aiResult.reason, selectedRecord: matchedRecord, selectedValue: matchedValue, outputVariable: outputVariableName }
    );
  }
  ...
  const stepOutput = {
    ...
    method: httpMethod,
    candidateCount: candidates.length,
    selectedIndex: aiResult.matchIndex,
    confidence: aiResult.confidence,
    selectionReason: aiResult.reason,
