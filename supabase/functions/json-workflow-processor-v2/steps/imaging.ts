...
export async function executeImaging(
  ...
  const rawDetailLineId = config.detailLineId || "";
  const rawBillNumber = config.billNumber || "";
  const detailLineId = resolveTemplate(rawDetailLineId);
  const billNumber = resolveTemplate(rawBillNumber);
  const bucketId = config.bucketId || "";
  const documentTypeId = config.documentTypeId || "";

  console.log("[IMAGING-RESOLVE] mode:", mode);
  console.log("[IMAGING-RESOLVE] raw detailLineId template:", JSON.stringify(rawDetailLineId));
  console.log("[IMAGING-RESOLVE] resolved detailLineId:", JSON.stringify(detailLineId));
  console.log("[IMAGING-RESOLVE] raw billNumber template:", JSON.stringify(rawBillNumber));
  console.log("[IMAGING-RESOLVE] resolved billNumber:", JSON.stringify(billNumber));
  console.log("[IMAGING-RESOLVE] contextData.billNumber:", JSON.stringify(contextData.billNumber));
  console.log("[IMAGING-RESOLVE] contextData.detailLineId:", JSON.stringify(contextData.detailLineId));
  console.log(
    "[IMAGING-RESOLVE] contextData.response keys:",
    contextData.response && typeof contextData.response === "object"
      ? Object.keys(contextData.response)
      : "(no response object)"
  );
  if (contextData.response && typeof contextData.response === "object") {
    console.log("[IMAGING-RESOLVE] contextData.response:", JSON.stringify(contextData.response).slice(0, 500));
  }
  ...
  const getRetrievalMode = mode === "get" ? (config.getRetrievalMode || "single") : null;
  const effectiveAction = mode === "get" && getRetrievalMode === "all" ? "list" : mode;
  ...
  if (mode === "get" && billNumber) payload.billNumber = billNumber;

  if (mode === "get" && effectiveAction === "get" && !detailLineId) {
    throw new Error(
      "Imaging GET (single) requires a Detail Line ID. Provide it in the step config or switch Retrieve to 'All documents' and provide a Bill Number."
    );
  }

  if (mode === "get" && effectiveAction === "list" && !detailLineId && !billNumber) {
    throw new Error(
      "Imaging GET (all documents) requires either a Detail Line ID or a Bill Number."
    );
  }
  ...
  if (effectiveAction === "list" && Array.isArray(result.documents)) {
    contextData.imagingDocuments = result.documents;
    contextData.imagingDocumentUrls = result.documents
      .map((d: any) => d.documentUrl)
      .filter((u: any) => !!u)
      .join(",");
    contextData.imagingDocumentCount = result.documents.length;
    console.log(
      `[IMAGING-GET-LIST] Stored ${result.documents.length} document(s) on context. URLs:`,
      contextData.imagingDocumentUrls
    );
  }

  if (mode === "get") {
    const attachmentTemplate = config.attachmentFilenameTemplate || "";
    const resolvedAttachmentName = attachmentTemplate ? resolveTemplate(attachmentTemplate) : "";
    const sanitizeFilename = (name: string): string => {
      const cleaned = (name || "").replace(/[\\/:*?"<>|\r\n\t]+/g, "_").trim();
      if (!cleaned) return "";
      return /\.[a-zA-Z0-9]{2,5}$/.test(cleaned) ? cleaned : `${cleaned}.pdf`;
    };
    const overrideName = sanitizeFilename(resolvedAttachmentName);
    if (attachmentTemplate) {
      console.log(
        `[IMAGING-GET-ATTACH] attachmentFilenameTemplate="${attachmentTemplate}" resolved="${resolvedAttachmentName}" final="${overrideName}"`
      );
    }
    ...
    if (effectiveAction === "list" && Array.isArray(result.documents)) {
      for (const d of result.documents) {
        if (d && d.documentUrl) {
          const fallback = d.originalFilename || d.filename || `document-${d.id || targets.length + 1}.pdf`;
          const filename = overrideName
            ? (result.documents.length > 1
                ? overrideName.replace(/(\.[^.]+)?$/, (_ext, ext) => `_${targets.length + 1}${ext || ".pdf"}`)
                : overrideName)
            : fallback;
          targets.push({ url: d.documentUrl, filename });
        }
      }
    } else if (result.documentUrl) {
      const fallback = result.originalFilename || result.filename || `document-${result.documentId || 1}.pdf`;
      targets.push({
        url: result.documentUrl,
        filename: overrideName || fallback,
      });
    }

    if (targets.length > 0) {
      console.log(`[IMAGING-GET-ATTACH] Downloading ${targets.length} document(s) for potential email attachment`);
      const attachments: { filename: string; base64: string }[] = [];
      for (const t of targets) {
        try {
          const dlResp = await fetch(t.url);
          if (!dlResp.ok) {
            console.error(`[IMAGING-GET-ATTACH] Failed to fetch ${t.url}: ${dlResp.status}`);
            continue;
          }
          ...
          attachments.push({ filename: t.filename, base64 });
          console.log(`[IMAGING-GET-ATTACH] Downloaded ${t.filename} (${buf.length} bytes)`);
        } catch (err) {
          console.error(`[IMAGING-GET-ATTACH] Exception downloading ${t.url}:`, err);
        }
      }
      if (attachments.length > 0) {
        const existing = Array.isArray(contextData.imagingAttachments) ? contextData.imagingAttachments : [];
        contextData.imagingAttachments = existing.concat(attachments);
        console.log(`[IMAGING-GET-ATTACH] Appended ${attachments.length} attachment(s); total on context.imagingAttachments is now ${contextData.imagingAttachments.length}`);
      }
    }
  }
