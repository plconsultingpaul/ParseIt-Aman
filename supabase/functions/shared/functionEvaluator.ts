...
export function evaluateDateTimeMergeFunction(logic: DateTimeMergeFunctionLogic, data: Record<string, any>): string {
  ...
  if (!dateParts) return '';
  const timeParts = parseTimePart(timeVal) || { hours: 0, minutes: 0, seconds: 0 };
...
You are knowledgeable about addresses worldwide. Always provide your best answer based on the address information given. Do not refuse or say you cannot determine the value.`;
  ...
  try {
    ...
    if (!text || /^\[.*\]$/.test(text.trim()) || lowerText.includes('cannot determine') || lowerText.includes('unable to determine') || lowerText.includes('i cannot') || lowerText.includes("i can't") || lowerText.includes('not able to') || lowerText.includes('not enough information') || lowerText.includes('could not determine') || lowerText.includes('no valid')) {
      return defaultValue || '';
    }
    return text;
  } catch (error) {
