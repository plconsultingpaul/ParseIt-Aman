...
You are knowledgeable about addresses worldwide. Always provide your best answer based on the address information given. Do not refuse or say you cannot determine the value.`;
  ...
  try {
    ...
    if (!text || lowerText === 'unknown' || /^\[.*\]$/.test(text.trim()) || lowerText.includes('cannot determine') || lowerText.includes('unable to determine') || lowerText.includes('i cannot') || lowerText.includes("i can't") || lowerText.includes('not able to') || lowerText.includes('not enough information') || lowerText.includes('could not determine') || lowerText.includes('no valid')) {
      console.log(`[AddressLookup] AI could not resolve (text="${text}"), returning default: "${defaultValue || ''}"`);
      return defaultValue || '';
    }
